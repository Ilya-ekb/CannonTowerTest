...
public class ParabolicAiming : IAiming
{
    private const int maxIterations = 6;
    private const float timeEpsilon = 0.01f;

    public Vector3 GetAimDirection(Transform shootPoint, Monster target, float projectileSpeed, Vector3 gravity)
    {
        if (target == null || projectileSpeed <= 0f)
        ...
        if (TrySolveBallisticIntercept(origin, targetPos, targetVel, gravity, projectileSpeed, out var launchVelocity))
            return launchVelocity.normalized;
    ...
    private bool TrySolveBallisticArc(Vector3 origin, Vector3 target, Vector3 gravity, float speed,
        out Vector3 launchVelocity)
    {
        launchVelocity = Vector3.zero;
